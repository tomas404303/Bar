...
def listar_productos():
    ...
    try:
        query = """
            SELECT 
                p.id, p.codigoProducto, p.nombreProducto, p.categoria,
                FORMAT(p.costo, 'N0', 'es-ES'), FORMAT(p.precioVenta, 'N0', 'es-ES'), p.estado, s.nombre AS sede
            FROM productos p
...
@router.get("/{codigoProducto}/{sede}")
def obtener_producto(codigoProducto: str, sede: int):
    db = connect_to_sqlserver()
    cursor = db.cursor()
    try:
        query = "SELECT * FROM productos WHERE codigoProducto = ? AND sede = ?"
        cursor.execute(query, (codigoProducto, sede))
